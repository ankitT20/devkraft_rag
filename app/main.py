...
async def root():
    ...
    return HealthResponse(status="ok", message="DevKraft RAG API is running")


@app.get("/health", response_model=HealthResponse)
async def health():
    """Health check endpoint."""
    return HealthResponse(status="ok", message="All services are operational")
...
async def query(request: QueryRequest):
    ...
    try:
        ...
        response, thinking, chat_id, sources = rag_service.query(
            user_query=request.query, model_type=request.model_type, chat_id=request.chat_id
        )

        return QueryResponse(response=response, thinking=thinking, chat_id=chat_id, sources=sources)
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        return IngestionResponse(success=success, message=message, filename=file.filename)
...
async def ingest_url(request: IngestionRequest):
    ...
    """
    try:
        app_logger.info(f"Received URL ingestion request: {request.url}")

        # Ingest the URL directly
        success, message = ingestion_service.ingest_document(request.url)

        return IngestionResponse(success=success, message=message, filename=request.url)

    except Exception as e:
        error_logger.error(f"URL ingestion endpoint failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
