    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            if self.api_call_count % 50 == 0:
                app_logger.info(
                    f"Rate limiting: Applied 10 second delay after {self.api_call_count} API calls"
                )
                time.sleep(10)
...
class LocalEmbedding:
    ...
    def _embed_with_lmstudio(self, text: str) -> List[float]:
        ...
        try:
            response = requests.post(
                ...
                json={"model": self.local_model, "input": text},
                timeout=30,
            )
    ...
    def _embed_with_hf(self, text: str) -> List[float]:
        ...
        try:
            ...
            response = self.hf_client.feature_extraction(text=text, model=self.hf_model)
