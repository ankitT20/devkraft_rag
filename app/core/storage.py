...
class QdrantStorage:
    ...
    def __init__(self):
        ...
        try:
            ...
            self._ensure_collection(
                self.cloud_client, self.cloud_collection, settings.gemini_embedding_dim
            )
            self._ensure_collection(
                self.cloud_client, self.cloud_docker_collection, settings.local_embedding_dim
            )
            app_logger.info(f"Initialized Qdrant Cloud client: {settings.qdrant_cloud_url}")
            app_logger.info(
                f"Cloud collections: {self.cloud_collection}, {self.cloud_docker_collection}"
            )
        ...
        try:
            ...
            self._ensure_collection(
                self.docker_client, self.docker_collection, settings.local_embedding_dim
            )
        ...
        try:
            ...
            results = self.cloud_client.scroll(
                ...
                scroll_filter=Filter(
                    must=[FieldCondition(key="md5", match=MatchValue(value=md5_hash))]
                ),
                limit=1,
            )
    ...
    def store_embeddings_cloud(
        ...
        try:
            ...
            self.cloud_client.upsert(collection_name=self.cloud_collection, points=points)
    ...
    def store_embeddings_docker(
        ...
        for i, (embedding, text) in enumerate(zip(embeddings, texts)):
            point_id = str(uuid7())  # Use UUIDv7 for monotonic growth
            payload = {"text": text, "md5": md5_hash, "metadata": metadata[i] if metadata else {}}
            points.append(PointStruct(id=point_id, vector=embedding, payload=payload))
        ...
        if self.docker_available:
            try:
                app_logger.info(
                    f"Storing {len(embeddings)} embeddings in docker collection (localhost)"
                )
                self.docker_client.upsert(collection_name=self.docker_collection, points=points)
                app_logger.info(
                    f"Successfully stored {len(points)} points in docker collection (localhost)"
                )
                docker_success = True
            except Exception as e:
        ...
        try:
            ...
            self.cloud_client.upsert(collection_name=self.cloud_docker_collection, points=points)
            app_logger.info(
                f"Successfully replicated {len(points)} points to cloud docker collection"
            )
            cloud_success = True
        except Exception as e:
    ...
    def search_cloud(self, query_vector: List[float], limit: int = 4) -> List[Dict]:
        ...
        try:
            ...
            results = self.cloud_client.search(
                collection_name=self.cloud_collection, query_vector=query_vector, limit=limit
            )
    ...
    def search_docker(self, query_vector: List[float], limit: int = 4) -> List[Dict]:
        ...
        if self.docker_available:
            try:
                ...
                results = self.docker_client.search(
                    collection_name=self.docker_collection, query_vector=query_vector, limit=limit
                )
                ...
                app_logger.info(
                    f"Found {len(search_results)} results in docker collection (localhost)"
                )
                return search_results
        ...
        try:
            ...
            results = self.cloud_client.search(
                collection_name=self.cloud_docker_collection, query_vector=query_vector, limit=limit
            )
