...
class ChatStorageService:
    ...
    def save_chat_history(self, chat_id: str, messages: List[Dict], model_type: str) -> bool:
        ...
        if self.mongo_available:
            try:
                self.collection.replace_one({"chat_id": chat_id}, data, upsert=True)
        ...
        if self.mongo_available:
            try:
                ...
                if data:
                    ...
                    data.pop("_id", None)
                    app_logger.info(
                        f"Retrieved full chat history from MongoDB for chat_id={chat_id}"
                    )
                    return data
            except Exception as e:
    ...
    def get_recent_chats(self, limit: int = 10) -> List[Dict]:
        ...
        if self.mongo_available:
            try:
                chats = list(
                    self.collection.find({}, {"_id": 0})  # Exclude MongoDB's _id field
    ...
    def _get_recent_chats_from_json(self, limit: int) -> List[Dict]:
        ...
        chat_files = sorted(
            chat_folder.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True
        )
